...
def stage1(args: argparse.Namespace, config: Dict[str, Any], keyword_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    research_md_parts = [
        ...
        f"Goal: Identify high-ROI keywords, audience intent, and emotional resonance to drive organic clicks and conversions.",
    ...
    for cluster in clusters:
        breakdown_lines = [
            ...
            f"  - Pinterest visual angles: {', '.join(cluster['pinterest_angles'][:3])}",
            f"  - Meta caption hooks: {', '.join(cluster['meta_hooks'][:3])}",
            f"  - Product compatibility: {cluster['product_compatibility']}",
        ]
        research_md_parts.extend(breakdown_lines)
        research_md_parts.append("")
    ...
    justification = (
        ...
        f"and emotional drivers around {', '.join(winning_cluster['emotional_drivers'])}. "
    ...
    research_md_parts.extend([
        "## Winner Selection & Justification",
        fill(justification, width=100),
        "",
        "### Next Actions",
        "1. Draft the unified keyword cluster asset.",
        "2. Map emotional intent and persona triggers.",
        "3. Finalize CTR forecasts and Pinterest visual plans.",
    ])

    write_text_file(output_dir / "market_research.md", "\n".join(research_md_parts))
    ...
    write_text_file(output_dir / "persona_profile.md", "\n".join(persona_lines))
    ...
    write_text_file(output_dir / "seo_brief.md", "\n".join(seo_lines))

    competitor_lines = ["# Competitor Audit"]
    ...
    write_text_file(output_dir / "competitor_audit.md", "\n".join(competitor_lines))
    ...
    context = {
        ...
        "lookback_days": args.lookback_days,
        "winning_cluster": {
            "id": winning_cluster["id"],
            "label": winning_cluster["label"],
            "core_emotion": winning_cluster["core_emotion"],
            "conversion_potential": winning_cluster["conversion_potential"],
            "top_keywords": winning_cluster["top_keywords"],
            "total_volume": winning_cluster["total_volume"],
            "avg_difficulty": winning_cluster["avg_difficulty"],
            "avg_ctr": winning_cluster["avg_ctr"],
            "meta_hooks": winning_cluster["meta_hooks"],
            "pinterest_angles": winning_cluster["pinterest_angles"],
        },
        "primary_keyword": primary_keyword,
        "seo_title": seo_title,
...
def stage2(args: argparse.Namespace, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    write_text_file(output_dir / "outline.md", "\n".join(outline_lines))

    affiliate_prompt_path = output_dir / "affiliate_link_prompt.txt"
    ...
    draft_sections = ["# {title}".format(title=context.get("seo_title", "Devotion Blueprint"))]
    draft_sections.extend(section_summaries)
    draft_sections.append("## Pinterest Visual Hook")
    ...
    draft_sections.append("")
    draft_sections.extend(faq_lines)
    draft_sections.append("## Final Empowerment Close")
    draft_sections.append(
        fill(
    ...
    draft_sections.append(concluding_paragraph)
    ...
    words = re.findall(r"[A-Za-z0-9']+", draft_content)
    ...
    primary_density = round(compute_keyword_density(draft_content, primary_keyword) * 100, 2)
    secondary_density = {
        kw: round(compute_keyword_density(draft_content, kw) * 100, 2) for kw in secondary_keywords
    }
    grade_level = flesch_kincaid_grade(draft_content)
    ...
    headings = [line for line in draft_content.splitlines() if line.startswith("## ")]
    keyword_in_headings = sum(1 for heading in headings if primary_keyword.lower() in heading.lower())
    checklist_lines.append(
        f"| Headers include query keywords | ≥50% | {keyword_in_headings}/{len(headings)} contain primary keyword | Continue refining headings |"
    )
    emotional_words = ["fear", "desire", "hope", "reassurance"]
    emotional_presence = {
        word: (word in draft_content.lower()) for word in emotional_words
    }
    checklist_lines.append(
    ...
    write_text_file(output_dir / "optimization_checklist.md", "\n".join(checklist_lines))

    slug = slugify(context["seo_title"])

    final_markdown = draft_content
    final_path = output_dir / "final_draft.md"
    ...
    (output_dir / "seo_package.json").write_text(json.dumps(seo_package, indent=2), encoding="utf-8")
    ...
    write_text_file(output_dir / "image_brief.md", "\n".join(image_brief_lines))
...
def stage3(args: argparse.Namespace, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for platform, details in stage3_config["platforms"].items():
        ...
        distribution_lines.append("- Tracking UTM: " f"?tid=understandingman_social_{slug}")
        distribution_lines.append("")

    write_text_file(output_dir / "distribution_plan.md", "\n".join(distribution_lines))
...
def stage4(args: argparse.Namespace, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    write_text_file(output_dir / "analytics_plan.md", "\n".join(analytics_lines))
...
def stage5(args: argparse.Namespace, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    write_text_file(output_dir / "growth_loop.md", "\n".join(growth_lines))
